#!/bin/bash
# ── 실행 / 종료 ──────────────────────────────────────────────────
#   실행        : ./needaimbot.sh            # 이 터미널에서 포그라운드 실행
#   종료        : Ctrl-C                      # 클럭 자동 복원됨
#   백그라운드  : nohup ./needaimbot.sh >/tmp/needaimbot.log 2>&1 &
#                 종료 시  →  pkill -INT -f build/bin/Release/simple_inference
# ────────────────────────────────────────────────────────────────
cd "$(dirname "$(readlink -f "$0")")/inference_pc"  # 스크립트 위치 기준 — 폴더 이동에 안전

# 세션 동안만 GPU/CPU 클럭을 최대로 고정 (지터 제거 + 추론 1.35ms 실현).
...
restore_clocks() {
    ...
    if [ -f "$CLOCK_STORE" ]; then
        sudo jetson_clocks --restore "$CLOCK_STORE" \
            && echo "[needaimbot] jetson_clocks 복원됨 (DVFS 재활성화)"
    fi
}
...
echo "[needaimbot] jetson_clocks 고정 중..."
...
echo "[needaimbot] jetson_clocks 고정 완료"
