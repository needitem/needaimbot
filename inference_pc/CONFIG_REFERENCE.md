...
| 키 | 기본 | 의미 |
|---|---|---|
| `aim_kp_x` / `aim_kp_y` | 0.55 / 0.6 | 우클릭 조준 비례 게인(가로/세로). 높이면 빠르지만 오버슛↑. |
| `aim_softness_x` / `_y` | 11.0 / 10.0 | 비선형 P 소프트닝. 클수록 먼 거리에서 완만. |
| `aim_kd_x` / `_y` | 0.18 / 0.22 | 미분 게인. 흔들림·오버슛 억제. 너무 크면 둔해짐. |
...
**Target tracking / stickiness / feedforward**
| 키 | 기본 | 의미 |
|---|---|---|
