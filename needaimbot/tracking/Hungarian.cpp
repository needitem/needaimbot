...
double HungarianAlgorithm::Solve(vector<vector<double>>& DistMatrix, vector<int>& Assignment)
{
	// Safety checks
	if (DistMatrix.empty()) {
		std::cerr << "[Hungarian] ERROR: Empty distance matrix" << std::endl;
		Assignment.clear();
		return 0.0;
	}
	...
	if (nCols == 0) {
		std::cerr << "[Hungarian] ERROR: Distance matrix has no columns" << std::endl;
		Assignment.clear();
		return 0.0;
	}
	...
	if (nRows > 1000 || nCols > 1000) {
		std::cerr << "[Hungarian] WARNING: Very large matrix (" << nRows << "x" << nCols << ")" << std::endl;
		// Fallback to simple greedy assignment for very large matrices
		Assignment.clear();
	...
	try {
		for (unsigned int i = 0; i < nRows; i++) {
			...
			if (DistMatrix[i].size() != nCols) {
				std::cerr << "[Hungarian] ERROR: Inconsistent row size at row " << i << std::endl;
				delete[] distMatrixIn;
				delete[] assignment;
				Assignment.clear();
				return 0.0;
		...
		std::cerr << "[Hungarian] Exception filling distance matrix: " << e.what() << std::endl;
		delete[] distMatrixIn;
		delete[] assignment;
		Assignment.clear();
		return 0.0;
	} catch (...) {
		std::cerr << "[Hungarian] Unknown exception filling distance matrix" << std::endl;
		delete[] distMatrixIn;
		delete[] assignment;
		Assignment.clear();
		return 0.0;
